...
void Member::showAllAvailableSupporters(string hostID) {
    ...
    while(true) {
        ...
        if(availableList.isValidNumber(input)) {
            ...
            cout << "helolo" << checkCredit(skillRequest.getCreditPerHour());
            Request request;
...
void Member::updateCreditInFile(string userId, float newCreditPoint){
    ...
    while (std::getline(myFile, line))
    {
        ...
        if (temp_userID == userId_val)
        {
            float tempCreditPoint_val = std::stof(tempCreditPoint_val);
            tempCreditPoint = tempCreditPoint - newCreditPoint;
            lines.push_back(temp_userID + "," + temp_password + "," + temp_username + "," + tempFullname + "," + tempEmail + "," + tempPhoneNumber + "," + tempHomeAddress + "," + tempCity + "," + tempCreditPoint + "," + restOfLine);
    ...
    myFile.close();
    std::cout << "Credit point updated successfully.\n";
}
