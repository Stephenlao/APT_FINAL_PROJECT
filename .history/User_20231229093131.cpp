...
void AvailableList::showDetailSupporterDetail(string& userName, string hostID) {
    ...
    while (std::getline(myFile, line)) {
        ...
        if (userName == data[2]) {
            ...
            getSkillNameByOrderNumber(skillRatingSupporter);
...
void AvailableList::createRequest(vector<Skill*>& skillRatingSupporter, string hostID, string supporterID){
    string skillRequest;
            int skillNumber = 0;
...
string AvailableList::getSkillNameByOrderNumber(vector<Skill*>& skillRatingSupporter){
    string skillRequest;
    int skillNumber = 0;
    cout << "\n" << "PLease choose number of skill you want to book";
    cout << "\n" << "Enter number (or press x to quit): ";
