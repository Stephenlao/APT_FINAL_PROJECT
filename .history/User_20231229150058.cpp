    ...
    while(true) {
        ...
        if(availableList.isValidNumber(input)) {
            ...
            if (checkCredit(skillRequest.getCreditPerHour())){
                updateCreditInFile(hostID, skillRequest.getCreditPerHour());
                request.saveRequestDataToFile(hostID, getUserIdByName(userNameOfSupporter), skillRequest.getSkillName());
            } else {
