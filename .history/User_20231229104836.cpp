...
void Member::showAllAvailableSupporters(string hostID) {
    ...
    while(true) {
        ...
        if(availableList.isValidNumber(input)) {
            ...
            string skillRequest = availableList.getRequestSkillName(userNameOfSupporter);
            
            Request request;
            
            request.saveRequestDataToFile(hostID, userNameOfSupporter, skillRequest);
...
string Member::getUserIdByName(string user){
...
string AvailableList::getRequestSkillName(string& userName){
    string skillRequest;
    std::fstream myFile;
    string skillRating;
    myFile.open("members.dat", std::ios::in);
